...
inner_planets = [
    ...
    {'name': 'Venus', 'x': 300, 'y': -100},
    {'name': 'Earth', 'x': 500, 'y': -150},
    {'name': 'Mars', 'x': 700, 'y': -200}
]
outer_planets = [
    {'name': 'Jupiter', 'x': 100, 'y': -50},
    {'name': 'Saturn', 'x': 300, 'y': -80},
    {'name': 'Uranus', 'x': 500, 'y': -120},
    {'name': 'Neptune', 'x': 700, 'y': -160}
]
asteroids = []
bullets = []
power_ups = []
...
def draw_intro():
    ...
    line4 = "This speed, known as escape velocity, is a staggering 11.186 km/s."
    line5 = "After we conquer Earth's grip,"
    line6 = "we'll embark on a journey through our solar system's magnificent planets to be familiar with them."
...
def handle_events():
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if game_state in ('innerSpace', 'asteroidBelt', 'outerSpace'):
...
def update_game():
    ...
    if game_state == 'intro':
        ...
        # Display text
        display_centered_text("Game Over!", HEIGHT // 2 - 100, font_large, YELLOW)
        display_centered_text(f"Final Score: {score}", HEIGHT // 2 + 50, font_medium, YELLOW)
        display_centered_text("Press R to Restart", HEIGHT // 2 + 120, font_small, YELLOW)
        ...
        # Display text
        display_centered_text("Congratulations!", HEIGHT // 2 - 100, font_large, YELLOW)
        display_centered_text("You explored our solar system planets:", HEIGHT // 2 + 50, font_small, YELLOW)
        display_centered_text(f"the inner planets, asteroid belt, and the outer planets. Score: {score}", HEIGHT // 2 + 100, font_small, YELLOW)
        display_centered_text("Head back to our web to continue your journey beyond the solar system.", HEIGHT // 2 + 150, font_small, YELLOW)
        display_centered_text("and get your story with an exoplanet!", HEIGHT // 2 + 200, font_small, YELLOW)
